...
static void
mp4tree_box_print(const uint8_t *p, const uint8_t *type, size_t len, int depth)
{
    printf("%s--- Offset: %zu Length: %zu Type: %c%c%c%c\n", indent(depth, 1), p - g_options.content_buf, len, type[0], type[1], type[2], type[3]);
    printf("%s  Description: %s\n", indent(depth + 1, 0), get_box_desc(type));
}
...
static void
mp4tree_box_mdat_h264_print(const uint8_t *p, size_t len, int depth)
{
    const uint8_t *p_end = p + len;
    while (p < p_end) {
        uint32_t nal_length = get_u32(p);

        printf("%s--- Offset: %zu Length %u Type: H264 NAL\n", indent(depth, 1), p - g_options.content_buf, nal_length);
...
static void
mp4tree_box_mdat_hevc_print(const uint8_t *p, size_t len, int depth)
{
    ...
    int            depth)
{
    ...
    while (p < end) {
        ...
        if (mp4tree_match_filter(box_type)) {
            mp4tree_box_print(p, box_type, box_len, depth);
...
int main(int argc, char **argv)
{
    ...
    g_options.content_buf = buf;

    int fd = open(g_options.filename, 0);
